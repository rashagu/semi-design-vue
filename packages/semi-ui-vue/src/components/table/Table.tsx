...
function Table<RecordType extends Record<string, any>>() {
    const TableComp = defineComponent<NormalTableProps<RecordType>>((props, {}) => {
            ...
            return {
                ...
                getCellWidths: (flattenedColumns: ColumnProps<RecordType>[], flattenedWidths: BaseHeadWidth[] = null, ignoreScrollBarKey = false): number[] => {
                    if (Array.isArray(flattenedColumns) && flattenedColumns.length) {
                        flattenedWidths =
                          flattenedWidths == null && state.headWidths.length ?
                            flattenDeep(state.headWidths) :
                            [];
                        if (
        ...
        function getDerivedStateFromProps(props: NormalTableProps<RecordType>) {
            const willUpdateStates: Partial<NormalTableState<RecordType>> = {};
        ...
        watch([
            ...
            if (
              expandAllRows !== prevPropsExpandAllRows ||
              expandAllGroupRows !== prevPropsExpandAllGroupRows
            ) {
             ...
             */
            if (dataSource !== prevPropsDataSource || stateCachedColumns !== prevStateCachedColumns || stateCachedChildren !== prevStateCachedChildren) {
            ...
            if (Object.keys(states).length) {
                ...
                const handledProps: Partial<NormalTableState<RecordType>> = foundation.getCurrentPageData(stateDataSource, mergedStatePagination as TablePaginationProps, stateQueries as any);
                ...
                if ('dataSource' in states) {
                    if (props.defaultExpandAllRows && handledProps.groups && handledProps.groups.size ||
                      props.expandAllRows ||
                      props.expandAllGroupRows
                    ) {
        ...
        const renderSelection = (record = {} as any, inHeader = false): VueJsxNode => {
            const { rowSelection, disabledRowKeysSet } = state;
            ...
            if (rowSelection && typeof rowSelection === 'object') {
                ...
                if (inHeader) {
                    ...
                    const allIsSelected = foundation.allIsSelected(selectedRowKeysSet, disabledRowKeysSet, allRowKeys);
        ...
        const normalizeSelectionColumn = (props: { rowSelection?: TableStateRowSelection<RecordType>; prefixCls?: string } = {}) => {
        ...
        const normalizeScrollbarColumn = (props: { scrollbarWidth?: number } = {}): { key: 'column-scrollbar'; width: number; fixed: 'right' } => {
        ...
        const normalizeExpandColumn = (props: { prefixCls?: string; expandCellFixed?: ArrayElement<typeof strings.FIXED_SET>; expandIcon?: ExpandIcon } = {}) => {
            ...
            column.render =
              expandIcon !== false ?
                (text = '', record, index) =>
                  (adapter.mergedRowExpandable(record) ? renderExpandIcon(record) : null) :
                () => null;
        ...
        const addFnsInColumn = (column: ColumnProps = {}) => {
            if (column && (column.sorter || column.filters || column.useFullRender)) {
                ...
                const TitleNode = typeof rawTitle !== 'function' && <Fragment key={strings.DEFAULT_KEY_COLUMN_TITLE}>{rawTitle as VueJsxNode}</Fragment>;
        ...
        const toggleSelectAllRow = (status: boolean, e: TableSelectionCellEvent) => {
            foundation.handleSelectAllRow(status, e);
        };
        ...
        const renderTitle = (props: { title?: VueJsxNode | ((dataSource?: RecordType[]) => VueJsxNode); prefixCls?: string; dataSource?: any[] } = {}) => {
            let { title } = props;
            ...
            return isVNode(title) || typeof title === 'string' ? (
              <div class={`${prefixCls}-title`} x-semi-prop="title">{title}</div>
            ) : null;
        };
        ...
        const renderFooter = (props: { footer?: VueJsxNode | ((dataSource?: RecordType[]) => VueJsxNode); prefixCls?: string; dataSource?: RecordType[] } = {}) => {
            let { footer } = props;
            const { prefixCls, dataSource } = props;
        ...
        return () => {
            ...
            // pagination
            const tablePagination = state.pagination && propPagination ? renderPagination(state.pagination as any, propRenderPagination) : null;
            ...
            return (
              <div
                ref={rootWrapRef}
                class={classnames(className, `${prefixCls}-wrapper`)}
                  ...
                  <TableContextProvider {...tableContextValue}>
                      <Spin spinning={loading} size="large">
                          <div ref={wrapRef} class={wrapCls}>
                              ...
                              {renderTitle({ title: (props_ as any).title, dataSource: props_.dataSource, prefixCls: props_.prefixCls })}
