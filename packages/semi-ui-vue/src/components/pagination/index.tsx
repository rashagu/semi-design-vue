...
const Pagination = defineComponent<PaginationProps>((props, {}) => {
    ...
    function renderSmallPage(locale: PaginationLocale) {
        ...
        const page = (<div class={`${prefixCls}-item ${prefixCls}-item-small`}>{currentPage}/{totalPageNum} </div>);
          ...
          <LocaleConsumer componentName="Pagination">
              {
                  (locale: PaginationLocale) => (
                    size === 'small' ? renderSmallPage(locale) : renderDefaultPage(locale)
                  )
              }
          </LocaleConsumer>
        );
