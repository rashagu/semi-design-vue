...
const Demo = defineComponent(() => {
  const url = ref('https://lf3-static.bytednsdoc.com/obj/eden-cn/ptlz_zlp/ljhwZthlaukjlkulzlp/root-web-sites/dy.png');
  ...
  return ()=>(
    ...
    >
      <Avatar src={url.value} style={{ margin: 4 }} hoverMask={hoverMask} />
    </Upload>
  );
})
export default Demo;
...
import { Upload, Button, Toast } from '@kousum/semi-ui-vue';
import { IconUpload } from '@kousum/semi-icons-vue';
...
export default () => {
    ...
    return <Upload action={action} defaultFileList={defaultFileList} itemStyle={{ width: '300px' }} renderFileOperation={renderFileOperation}>
...
import { Upload, Image } from '@kousum/semi-ui-vue';
import { IconPlus } from '@kousum/semi-icons-vue';
...
import { Upload, Select, RadioGroup, Radio } from '@kousum/semi-ui-vue';
import { IconPlus, IconEyeOpened } from '@kousum/semi-icons-vue';
import { defineComponent, ref } from 'vue';
...
const Demo = defineComponent(() => {
  ...
  const hotSpotLocation = ref('end');
  return ()=>(
    <>
      <RadioGroup
        value={hotSpotLocation.value}
        type='button'
        onChange={e => hotSpotLocation.value = e.target.value}>
      ...
      <Upload
        ...
        accept="image/*"
        multiple
        hotSpotLocation={hotSpotLocation.value}
        defaultFileList={defaultFileList}
        onPreviewClick={handlePreview}
      >
        <IconPlus size="extra-large" />
...
const ManulUploadDemo = defineComponent(()=>{

  const uploadRef = ref();
  function manulUpload() {
  ...
  return ()=>{
    ...
    return (
      <div>
        ...
        >
          <Button icon={<IconPlus />} theme="light" style={{ marginRight: '8px' }}>
            选择文件
          </Button>
        </Upload>
...
export default ManulUploadDemo
```
...
const AsyncBeforeUploadDemo = defineComponent(()=>{
  const state = reactive({});
  let count = 0;
  function beforeUpload({ file, fileList }) {
    let result;
    return new Promise((resolve, reject) => {
      if (count > 1) {
        ...
        result = {
          ...
          status: 'validateFail',
          shouldUpload: false,
          validateMessage: `第${count + 1}个注定失败`,
        };
        count = count + 1;
        reject(result);
      }
...
export default AsyncBeforeUploadDemo
```
