...
const Comp = defineComponent(() => {
  ...
  const v = ref(['Shanghai']);

  return () => (
    <div style={{ margin: '10px', padding: '10px', width: '600px' }}>
      <Transfer dataSource={treeData} type="treeList" value={v.value} onChange={(value)=>v.value = value}></Transfer>
    </div>
  );
})
export default Comp;
