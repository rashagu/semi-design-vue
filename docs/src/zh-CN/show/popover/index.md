...
import { Popover, Space, Empty } from '@kousum/semi-ui-vue';
...
const content = (
    <Empty
        ...
        style={{ width: '400px', margin: '0 auto', display: 'flex', padding: '20px' }}
    />
);
...
import { Popover, Tag, Empty } from '@kousum/semi-ui-vue';
import { IllustrationSuccess, IllustrationSuccessDark } from '@kousum/semi-illustrations-vue';
            ...
            <div style={{ marginLeft: 80, whiteSpace: 'nowrap' }}>
                {tops.map((pos, index) => (
                    <Popover
                        ...
                        key={index}
                    >
                        <Tag style={{ margin: 8, padding: '20px' }}>{Array.isArray(pos) ? pos[1] : pos}</Tag>
                    </Popover>
                ))}
            </div>
            <div style={{ width: 80, float: 'left' }}>
                {lefts.map((pos, index) => (
                    <Popover
                        ...
                        key={index}
                    >
                        <Tag style={{ margin: 8, padding: '20px', width: 60 }}>{Array.isArray(pos) ? pos[1] : pos}</Tag>
                    </Popover>
                ))}
            </div>
...
```jsx live=true
import { Popover, Button, RadioGroup, Radio, Empty } from '@kousum/semi-ui-vue';
import { IllustrationSuccess, IllustrationSuccessDark } from '@kousum/semi-illustrations-vue';
...
const Demo = defineComponent(() => {
  ...
  return () => (
    <Popover visible={visible.value} content={content} trigger="custom">
      <RadioGroup type='button' onChange={(e) => visible.value = (e.target.value)} value={visible.value}>
...
function Demo() {
    ...
    return (
        <div style={{ paddingLeft: '40px' }} className="tag-margin-right">
